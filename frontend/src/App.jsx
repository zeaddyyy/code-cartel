...
function EvidenceImage({ snapshotPath, alt = "Evidence snapshot", className = "table-thumb" }) {
  ...
  return <>
    <button className="evidence-thumb-button" type="button" onClick={() => setOpen(true)} title="Open full-size evidence image"><img className={className} src={src} alt={alt} loading="lazy" onError={(event) => { event.currentTarget.style.opacity = "0.35"; }} /></button>
...
function App() {
  ...
  const [cameraForm, setCameraForm] = useState({ camera_id: "", name: "", location_name: "", latitude: "", longitude: "" });
  ...
  const addCamera = async (event) => {
    ...
    const response = await fetch(`${API}/api/cameras`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...cameraForm, latitude: Number(cameraForm.latitude), longitude: Number(cameraForm.longitude) }) });
    if (!response.ok) { setError("Camera could not be added"); return; }
    setCameraForm({ camera_id: "", name: "", location_name: "", latitude: "", longitude: "" }); load();
  };
  const deleteCamera = async (id) => { if (!window.confirm("Delete this camera and its dependent records?")) return; await fetch(`${API}/api/cameras/${id}`, { method: "DELETE" }); load(); };
  const editCamera = async (camera) => { const name = window.prompt("Camera name", camera.name || ""); if (name === null) return; await fetch(`${API}/api/cameras/${camera.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) }); load(); };
  ...
  return <div className="app command-shell">
    ...
    <div className="workspace">
      ...
      </main><footer className="app-footer"><span>NetraX Command Center</span><span>Government operations interface · evidence-aware analytics</span><span>v1.0 · {new Date().getFullYear()}</span></footer>
    </div>
  </div>;
}
...
function PageView({ page, cameras, detections, visibleCameras, visibleDetections, error, online, vehicles, anprReads, speedViolations, geoCameras, mapPosition }) {
  ...
  if (page === "Alerts") return <AlertsPage PageHeading={PageHeading} />;
